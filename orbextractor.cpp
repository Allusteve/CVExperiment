...

{
    ...
    for (int level = 0; level < nlevels; ++level) {
        ...
        // 得到每一层图像进行特征检测区域上下左右坐标

        // 相当于不检测边沿区域，边沿大小为EDGE_THRESHOLD - 3
...

        {
            ...
            //之所以这里是减去3是因为上面定义的边沿大小为EDGE_THRESHOLD - 3
...

            {
                ...
                //假如使用默认的阈值没有检测到特征点，就使用最小阈值再次进行检测
...

{
...

    {
        ...
        // 当划分得到的节点数达到最大或者每个节点内只有一个特征点时停止划分
